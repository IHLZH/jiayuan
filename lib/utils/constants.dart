class Constants {
  Constants._();

  //高德地图安卓端key
  static const String Gao_DE_MAP_KEP_ANDROID =
      "deec9d608ddc51b91c745ba02af59a96";

  //用户名称
  static const String SP_USER_NAME = "SP_USER_NAME";

  //用户密码
  static const String SP_USER_PWD = "SP_USER_PWD";

  //当前最新的版本号
  static const String SP_NEW_APP_VERSION = "SP_NEW_APP_VERSION";

  //鉴权token
  static const String SP_TOKEN = "SP_TOKEN";

  //是否开发模式
  static const bool IS_Production = true;

  //本地sqlite数据库
  static const String DB_NAME = "jiayuan.db";

  //后台网址
  // static const String BACKEND_URL = "http://10.7.89.161:3000";
  static const String BACKEND_URL = "http://62.234.165.111:3000";

  //仓库地址
  static const String REPO_URL = "https://codeup.aliyun.com/6708806c1c80af57f902d17f/aip-semester5-2022/family-bond";

  //客服ID
  static const int KF_ID = 19;

  //软件版本
  static const String APP_VERSION = "v1.0.0";

  //开发人员列表
  static const List<String> DEVELOPER_LIST = [
    "xjl",
    "lzh",
    "ywl",
    "cxy",
    "wsh",
    "yzl",
    "rh",
    "bxp",
  ];
}
