...
class CommissionApi {
  ...
  //委托接取首页推荐委托
  Future<List<CommissionData1>> getRecommendCommission(
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 0; i < total; i++) {
          commissionList.add(
              CommissionData1.fromJson(response.data['data'][i]));
        }
      }
    } catch (e) {
  ...
  //搜索委托
  Future<List<CommissionData1>> getSearchCommission(
      Map<String, dynamic>? param) async {
    List<CommissionData1> commissionList = [];
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        for (int i = 0; i < total; i++) {
          commissionList.add(
              CommissionData1.fromJson(response.data['results'][i]));
        }
      }
    } catch (e) {
      print("网络错误error:" + e.toString());
  ...
  Future<List<CommissionData1>> getCommission(
      {required String path, required Map<String, dynamic> param}) async {
    List<CommissionData1> commissionList = [];
